...
class Usuario_Controller:
    ...
    def save(self):
        try:
            nome, email, data_nascimento, cidade, perfil = (
                self.view.ler_dados_usuario()
            )
            usuario = Usuario(
                None,nome,email,Data_Utils.string_para_data(data_nascimento),cidade,perfil
            )
            self.dao.save(usuario)
            self.get_all()
            self.view.exibir_mensagem(Idioma.t("usuario.cadastro_sucesso"))
        ...
        try:
            if self.usuario_selecionado is None:
                self.view.exibir_mensagem(
                    (Idioma.t("usuario.selecione_usuario")),
                    False
                )
                return
            nome, email, data_nascimento, cidade, perfil = (
            ...
            self.get_all()
            self.view.exibir_mensagem(
                (Idioma.t("usuario.atualizado_sucesso"))
            )
        except ValueError as e:
            self.view.exibir_mensagem(
                (f"{Idioma.t('comum.erro_prefixo')}{Idioma.t(str(e))}", False),
                False
            )
    def delete(self):
        if self.usuario_selecionado is None:
        ...
        try:
            ...
            if sucesso:
                ...
                self.get_all()
                self.view.exibir_mensagem(
                    (Idioma.t("usuario.exclusao_sucesso"))
                )
            else:
                self.view.exibir_mensagem(
                    (Idioma.t("usuario.nao_encontrado")),
                    False
                )
        except Exception:
            self.view.exibir_mensagem(
                (Idioma.t("usuario.problema_exclusao")),
                False
            )