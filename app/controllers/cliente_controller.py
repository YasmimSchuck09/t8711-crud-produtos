    ...
    def update(self):
        try:
            if self.cliente_selecionado is None:
                self.view.exibir_mensagem(Idioma.t("cliente.selecione_cliente"), False)
                return
            nome, data_nascimento, limite_credito, cidade = self.view.ler_dados_cliente()
            ...
            self.get_all()
            self.view.exibir_mensagem(Idioma.t("cliente.atualizacao_sucesso"))
        except ValueError as e:
            self.view.exibir_mensagem(f"{Idioma.t('comum.erro_prefixo')}{Idioma.t(str(e))}", False)
    ...
    def delete(self):
        ...
        try:
            ...
            if sucesso:
                ...
                self.view.limpar_campos()
                self.get_all()
                self.view.exibir_mensagem(Idioma.t("cliente.exclusao_concluida"))
            else:
                self.view.exibir_mensagem(Idioma.t("cliente.nao_encontrado"), False)
        except Exception as e:
            self.view.exibir_mensagem(Idioma.t("cliente.problema_exclusao"), False)
